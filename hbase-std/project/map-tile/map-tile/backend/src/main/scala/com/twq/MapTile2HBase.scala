...
object MapTile2HBase {
  def main(args: Array[String]): Unit = {
    ...
    (startLevel.toInt to endLevel.toInt).zip(partitionsPerLevel).par.foreach { case (level, partitions) =>
      ...
      encodedTileRDD.foreachPartition { iterator =>
        ...
        iterator.foreach {case (tile, encodedTile) =>
          val put = new Put(Bytes.toBytes(s"${tile.reverse}_${level}")) //最终的rowkey：反转切片的名称_level
