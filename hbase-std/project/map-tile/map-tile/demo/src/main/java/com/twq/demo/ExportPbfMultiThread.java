...
class OneLevelTileTask implements Runnable {
    ...
    public OneLevelTileTask(List<Map<String, String>> polygons, int level, String path, String suffixName) {
...
public class ExportPbfMultiThread {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Pattern pattern = Pattern.compile(
                        ...
                        "([-.\\d]+)," +
                        "(POLYGON\\(\\([^\\\"]+\\)\\))");
        InputStream inputStream = ExportPbfMultiThread.class.getClassLoader().getResourceAsStream("hz_building.csv");
        ...
        int endLevel = 16; // 终止等级(小分辨率)
        //每隔level进行串行跑  ---> 每隔level并行跑
        for (int level = startLevel; level <= endLevel; level++) {
            executorService.submit(new OneLevelTileTask(polygons, level, path, suffixName));
        }
    }
}
