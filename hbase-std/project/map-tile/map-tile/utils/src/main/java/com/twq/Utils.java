...
public class Utils {
    ...
    public static List<String> getTiles(Geometry polygon, int level) {
        ...
        Envelope env = polygon.getEnvelopeInternal();//得到外接矩形的最大最小x,y
        int startColumn = (int) Math.floor((env.getMinX() - Tile.orginX) / resolution / Tile.TILE_SIZE);//得到开始列对应切片x
    ...
    private static double tileXToX(long tileX, int level) {
        return pixel2Coordinate(tileX * Tile.TILE_SIZE, level);
    }

    private static double tileYToY(long tileY, int level) {
