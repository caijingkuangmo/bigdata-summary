package com.twq.hbase.spark.simple

import com.twq.hbase.spark.HBaseConnectionCache
...
object SparkHbaseTest {
  ...
  private def doWithHbaseOnDriver(sc: SparkContext) = {
  ...
  private def putData2Hbase(sc: SparkContext) = {
  ...
  private def getDataFromHbase(sc: SparkContext) = {
  ...
  private def cloneSnapshot(sc: SparkContext) = {
    val conf = HBaseConfiguration.create(sc.hadoopConfiguration)
