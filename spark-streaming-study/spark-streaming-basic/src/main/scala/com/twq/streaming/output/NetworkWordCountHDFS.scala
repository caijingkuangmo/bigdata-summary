...
object NetworkWordCountHDFS {
  def main(args: Array[String]) {
    ...
    wordCounts.repartition(1).map(x => {
      ...
      "hdfs://master:9999/user/hadoop-twq/spark-course/streaming/data/sequence/wordcount", "-hadoop")

    wordCounts.repartition(1).saveAsTextFiles("hdfs://master:9999/user/hadoop-twq/spark-course/streaming/data/text/wordcount")
