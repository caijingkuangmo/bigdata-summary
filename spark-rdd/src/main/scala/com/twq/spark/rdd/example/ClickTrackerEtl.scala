package com.twq.spark.rdd.example

import java.net.URL
import java.util.UUID

import com.esotericsoftware.kryo.Kryo
import com.twq.spark.rdd.{TrackerLog, TrackerSession}
...
object ClickTrackerEtl {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sessionRDD =
      parsedLogRDD.groupBy((log: TrackerLog)
          => log.getCookie.toString, partitioner).flatMapValues { case iter =>
    ...
    val labeledSessionRDD =
      sessionRDD.leftOuterJoin(cookieLabelRDD).map { case (cookie, (session, cookieLableOpt)) =>
  ...
  private def writeOutputdata(sc: SparkContext, trackerDataPath: String,
                              ...
                              labeledSessionRDD: RDD[TrackerSession]) = {
    ...
    parsedLogRDD.map((null, _)).saveAsNewAPIHadoopFile(trackLogOutputPath,
      ...
      classOf[AvroParquetOutputFormat[TrackerLog]], configuration)
  ...
  private def cutSession(sortedParsedLogs: Array[TrackerLog]): ArrayBuffer[ArrayBuffer[TrackerLog]] = {
    val sessionParsedLogsBuffer = ArrayBuffer[TrackerLog]()
    val initBuilder = ArrayBuffer.newBuilder[ArrayBuffer[TrackerLog]]
    val sessionLogResult =
      sortedParsedLogs.foldLeft((initBuilder, Option.empty[TrackerLog])) { case ((builder, preLog), currLog) =>
        val currLogTime = dateFormat.parse(currLog.getLogServerTime.toString).getTime
        // 如果当前的log的时间减掉上一个log的时间大于30分钟的话，则切割为一个新会话
        if (preLog.isDefined
          && currLogTime - dateFormat.parse(preLog.get.getLogServerTime.toString).getTime > sessionTimeInterval) {
  ...
  private def parseRawLog(line: String): Option[TrackerLog] = {
