...
/**
 *   RDD Java Api讲解: http://blog.51cto.com/7639240/category1.html
 *   特别强调看下： http://blog.51cto.com/7639240/1966131(rdd java api调用rdd scala api的原理)
 */
public class TopNJava {
    public static void main(String[] args) throws IOException {
        ...
        JavaRDD<String> rdd = sparkContext.textFile(urlPath); //读取文件，构建一个String类型的RDD
        JavaPairRDD<String, Long> wordCountRDD = rdd.filter(new Function<String, Boolean>() { //先进行filter
            @Override
            public Boolean call(String v1) throws Exception {
                return v1 != null && !v1.isEmpty();
            }
        }).flatMapToPair(new PairFlatMapFunction<String, String, Long>() { //先解析每一行记录，生成word count
            @Override
        ...
        }).reduceByKey(new Function2<Long, Long, Long>() { //对每一个word出现的总次数进行累加
            @Override
