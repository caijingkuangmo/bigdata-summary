...
object SparkSQLSupportHiveLocalTest {
  ...
  def main(args: Array[String]) {
    ...
    sql("CREATE DATABASE IF NOT EXISTS twq")
    ...
    sql("LOAD DATA LOCAL INPATH 'spark-sql-hive/src/main/resources/trackerSession' OVERWRITE INTO TABLE twq.tracker_session")

    // Queries are expressed in HiveQL
    sql("SELECT * FROM twq.tracker_session").show()

    // Aggregation queries are also supported.
    sql("SELECT COUNT(*) FROM twq.tracker_session").show()

    // The results of SQL queries are themselves DataFrames and support all normal functions.
    val sqlDF = sql("select cookie, count(*) as cnt from twq.tracker_session group by cookie")
