...
ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' 
STORED AS TEXTFILE
LOCATION '/user/hadoop-twq/hive-course/skewed';

--Skewed Table：含有倾斜的信息的表
INSERT OVERWRITE TABLE dml.skewed_single SELECT * FROM dml.skewed_test_tmp;

INSERT OVERWRITE TABLE dml.testskew SELECT * FROM dml.skewed_test_tmp;
...
LOAD DATA LOCAL INPATH '/home/hadoop-twq/hive-course/kv3.txt' OVERWRITE INTO TABLE pokes;
...
WITH SERDEPROPERTIES (
    ...
    "v.event_id.avro.schema.url" = "hdfs://master:9999/user/hadoop-twq/hive-course/sensor.avsc")
TBLPROPERTIES (
