...
--1、创建一张movie_links表
CREATE TABLE `douban.movie_links`(                            
   `movieid` string,                                
   `url` string)
STORED AS TEXTFILE;

--创建一张临时外部表，为了导数据用
CREATE EXTERNAL TABLE `douban.movie_links_tmp`(                 
   `url` string)
ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
STORED AS TEXTFILE
LOCATION "/user/hadoop-twq/hive-course/douban/links";
...
select m.movieid, m.moviename, commentnum / showtime as rate, commentscore from movie m  order by rate desc limit 10;
...
--4.1、数学函数round
select 
m.movieid, m.moviename, commentnum / showtime as rate, round(commentnum / showtime, 2) as round_rate, commentscore 
from movie m  
order by rate desc 
limit 10;
--4.2、集合函数
