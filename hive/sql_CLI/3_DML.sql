...
LOAD DATA LOCAL INPATH '/home/hadoop-twq/hive-course/employee.txt' INTO TABLE employee;

CREATE TABLE sensor_format LIKE sensor.sensor_format;
--1、path可以是一个文件目录
--2、原始的文件将move到表所在的LOCATION中
LOAD DATA INPATH 'hdfs://master:9999/user/hadoop-twq/hive-course/omneo' OVERWRITE INTO TABLE sensor_format;
LOAD DATA INPATH 'hdfs://master:9999/user/hadoop-twq/hive-course/omneo' INTO TABLE sensor_format;
...
STORED AS TEXTFILE
LOCATION 'hdfs://master:9999/user/hadoop-twq/hive-course/omneo-temp/';
...
STORED AS PARQUET;
--将sensor_tmp中的数据导入到sensor_partition中的数据导入到sensor分区year=2018, month=201805, day=20180508中
...
--数据量大的话则写到HDFS的文件中
INSERT OVERWRITE DIRECTORY '/user/hadoop-twq/hive-course/employee'
...
--数据量比较小的话则写到本地文件中
INSERT OVERWRITE LOCAL DIRECTORY '/home/hadoop-twq/hive-course/employee'
ROW FORMAT DELIMITED 
      FIELDS TERMINATED BY '\t' 
      COLLECTION ITEMS TERMINATED BY ',' 
      MAP KEYS TERMINATED BY ':'
